...
def computeSummary():#Add all the data from each episode in the season
    seasonSummary = dict()
    for e in episodeTitles:#Subsequent episodes and their data
        current =  json.load(open('Seasons\\26\\' + e + '.json'))#Get the data of current episode
    ...
    with open('Seasons\\26\\Summary.json', 'w') as f:
...
for episode in episodeLinks:
    ...
    with open('Seasons\\26\\' + titleFile + '.json', 'w') as f:
